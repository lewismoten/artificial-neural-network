(() => {
  ...
  module.exports = (network, expectedOutput, learnRate, momentum) => {
    ...
    if (isUndefined(hiddenLayer.previousWeightsDelta)) {

      hiddenLayer.previousWeightsDelta =
  ...
  function updateHiddenWeights(weight, x, y) {
    ...
    let previousWeightsDelta = this.layer.previousWeightsDelta[x][y];
    this.layer.weights[x][y] += delta + this.momentum * previousWeightsDelta;
    this.layer.previousWeightsDelta[x][y] = delta;
